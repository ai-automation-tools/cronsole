...
| Works on PC, not on phone | **Run `tailscale status` on the PC and read the phone's row first.** `offline, last seen 1d ago` is the whole answer: the phone's Tailscale client has gone to sleep (Android suspends the VPN service after an update, a reboot, or battery optimisation), and nothing on the PC can reach a device that is not on the tailnet. Opening the Tailscale app on the phone once wakes it — no toggle, no re-login. Before that check, `curl` on the PC proves nothing, because the PC is always on the tailnet. If the phone is *online* and it still fails, confirm MagicDNS is on and that the URL carries `http://` and `:8080` (Chrome's HTTPS-first mode drops the scheme you did not type). Seen 2026-09-03. |
