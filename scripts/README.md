...
| **`check-control-bytes.mjs`** | Fail on any literal control byte in a text file — tracked **and** untracked-but-not-ignored (a NUL once made a security-relevant file diff as binary; scanning only tracked files left new files invisible until the very commit that added them, see [#32](../docs/troubleshooting/README.md#32-a-rename-pass-silently-disables-the-back-compat-it-just-added--and-rewrites-the-tests-too)). Wired into CI as the `repo-hygiene` job. |
