...
param(
    ...
    # CRONSOLE_SITE_CLONE at yours; unset (or not a clone) falls back to %TEMP%, which is
    # the portable path and needs no setup.
    [string]$WorkDir = $(
        $tools = $env:CRONSOLE_SITE_CLONE
        if ($tools -and (Test-Path (Join-Path $tools '.git'))) { $tools } else { Join-Path $env:TEMP 'cronsole-site-publish' }
    ),
