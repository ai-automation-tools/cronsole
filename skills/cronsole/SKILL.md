...
| **REST-only** (no MCP tool) | bulk export `POST /api/tools/export/tasks`, **restore `POST /api/tools/restore/tasks`**, **run history `GET /api/tools/history`** (JSON/CSV), **health `GET /api/tools/task-health`**, connect-pack downloads `GET /api/tools/downloads[/:id]`, plus template import/export, save-as-template, sync, pairing |
...
| **`ExecutionLog` records runs Cronsole *performed*, not runs that *happened*** | Rows come from exactly two places: `POST /api/tasks/:id/run` and `NativeScheduler`. **A Windows task firing on its own schedule writes nothing**, and a manual Windows run's `SUCCESS` means *"the agent started it"* — fire-and-forget, with `durationMs` timing the trigger, not the work. So a Windows task's real health lives in **Windows** (`lastTaskResult`, `lastRunTime`, `numberOfMissedRuns`, carried on the sync snapshot); only Cronsole-native tasks can be judged from `ExecutionLog`. Anything reading it as a complete run history is wrong at dashboard scale. |
