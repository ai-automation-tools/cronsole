...
| Tier | Tools |
|:---|:---|
| **Read** | `list_tasks` · `list_templates` · `list_folders` · `list_platforms` · `list_claude_routines` · `get_task_history` · `list_platform_runs` · `get_run_output` · `list_run_history` · `get_task_health` · `get_diagnostics` · `export_task` (`format: native \| template`) · `list_task_archives` · `list_task_secrets` · `convert_schedule` |
| **Create** | `create_task` · `create_native_task` · `create_native_program_task` · `create_native_script_task` · `create_native_check_task` · `create_task_from_template` · `create_claude_routine` · `create_gemini_trigger` · `import_task` · `restore_task_archive` |
