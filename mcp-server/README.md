...
|:---|:---|:---|
| `list_tasks` | `GET /api/tasks` | List tracked tasks with schedule, status, next run, last result. Optional `platform` / `status` / `category` / `search` filters — `platform` now includes **`GEMINI_TRIGGERS`** (a full controller — run, pause, create and delete all reach real endpoints, and it reports real run outcomes; a trigger's schedule and prompt are fixed once it exists, so `update_task_schedule` and `update_task_action` are refused there; a `DISABLED` trigger there may have been paused by Gemini itself after repeated failures rather than by a person), and **`GITHUB_ACTIONS`** and **`VERCEL_CRON`**, both **read-only** sources: their tasks list and report health, and every verb that would change one is refused (`list_platforms` is the authority). `VERCEL_CRON` additionally never reports a last run result — Vercel publishes no cron run history, so its tasks are permanently `unknown` health rather than unhealthy. `status` includes **`MISSING`** — tracked by Cronsole but absent from the platform on the last sync (a native delete, or an offline agent / unreadable folder — indistinguishable from here); it self-heals to ACTIVE/DISABLED when the task reappears. |
...
| `list_templates` | `GET /api/templates` | Browse the catalog with each template's id, tags, target platforms, default schedule, and declared `{{placeholder}}` parameters. A Gemini template also carries **`agentTools`** — the reach the created trigger is granted. A tool with a `preset` names a *saved* MCP server on the connection, never a credential. |
...
| `create_task_from_template` | `POST /api/templates/:id/apply` | Create a real task from a template — server fills placeholders from `parameters`, converts the cron, and registers it. Optional `folder` picks the Windows Task Scheduler folder (default `\Cronsole`; `\Microsoft\` refused), and optional **`createFolder`** (default `false`) creates a missing one instead of refusing — the agent is elevated, so removing it again needs **administrator rights**, and any folder created is named in the response. Targets **Windows, Cronsole-native, `CLAUDE_CODE` and `GEMINI_TRIGGERS`**: a Claude template's command is a *prompt*, so applying one creates a real routine (optional `repositoryUrls` attaches its checkout) — which needs a Claude Code session on the backend's machine, else a `400`. A Gemini template is a prompt **plus a grant**: applying one creates a trigger Google runs unattended with the tools the template declares, so read `agentTools` and say what it grants first. A named preset that is not saved is refused **with the list**, never passed through. |
| `create_native_task` | `POST /api/tasks/native` | Create a Cronsole-**native** HTTP task run by the backend itself — no agent, no machine to be logged into. Takes a full job spec (`method` / `headers` / `body`), which is why it's separate from `create_task`, whose native path accepts only a URL. Native schedules are used **as given** (the backend owns the scheduler), so none of the Windows hourly-fallback risk applies. **Secrets:** any free-text field may carry `${secret.NAME}` instead of a credential — Cronsole substitutes it from the task's encrypted store at run time and redacts the value back out of the run log. **No tool here sets a value** (a tool call is a chat transcript); the create result lists what is still unset as `missingSecrets`, and `list_task_secrets` reports the state. |
| `create_native_program_task` | `POST /api/tasks/native` | Create a Cronsole-**native** task that **runs an existing program** on a schedule, executed by the backend itself. Real run results — exit code, duration and captured output — land in the task history, unlike a Windows task where a `SUCCESS` only means the agent accepted the start. **Runs wherever the backend runs**, which is inside the container on a Dockerized install; check `executionHost` on the Cronsole-native row of `GET /api/tools/platforms` when the environment is unknown. The command is tokenized server-side and run with **no shell**. *(Named `create_native_script_task` until 2026-08-15 — it never took a script.)* **Secrets:** any free-text field may carry `${secret.NAME}` instead of a credential — Cronsole substitutes it from the task's encrypted store at run time and redacts the value back out of the run log. **No tool here sets a value** (a tool call is a chat transcript); the create result lists what is still unset as `missingSecrets`, and `list_task_secrets` reports the state. |
| `create_native_script_task` | `POST /api/tasks/native` | Create a native task from a **script you supply**. Cronsole stores the body, writes it to a temp file at run time under a named `interpreter` (`powershell`·`pwsh`·`bash`·`sh`·`python`·`node` — a fixed list, never a path), runs it, and deletes it. Nothing needs to exist on the user's disk, which makes this the right tool when *you* are authoring the script. `node` is the safest interpreter: the backend runs on it, so it is present everywhere. **Secrets:** any free-text field may carry `${secret.NAME}` instead of a credential — Cronsole substitutes it from the task's encrypted store at run time and redacts the value back out of the run log. **No tool here sets a value** (a tool call is a chat transcript); the create result lists what is still unset as `missingSecrets`, and `list_task_secrets` reports the state. |
| `create_native_check_task` | `POST /api/tasks/native` | Create a monitoring **check**: `http` (assert on status, body text, or a dotted JSON path), `tcp` (port accepts a connection), `fileFresh` (a file was written recently — **missing counts as failed**), `diskFree`. Use over `create_native_task` when the point is to *verify* rather than to *trigger* — a check can fail a `200` that serves an error page. `fileFresh`/`diskFree` measure the **backend's** filesystem. **Secrets:** any free-text field may carry `${secret.NAME}` instead of a credential — Cronsole substitutes it from the task's encrypted store at run time and redacts the value back out of the run log. **No tool here sets a value** (a tool call is a chat transcript); the create result lists what is still unset as `missingSecrets`, and `list_task_secrets` reports the state. |
...
| `export_task` | `GET /api/tasks/:id/export[?format=]` | **Two formats, two questions.** `native` (default): Windows → Task Scheduler **XML**, native → Cronsole **JSON** — puts *this exact task* back on *this platform*. `format: 'template'`: a portable **Registry v1 template** that recreates the task on any install, importable via `POST /api/templates/import` — and it **drops platform-specific settings** (run-as account, run level, extra actions), so it is not a faithful backup. Template also works where native cannot: with the agent offline, and for a Claude routine whose definition lives at claude.ai. Writes nothing either way — unlike save-as-template, which adds a catalog row. The route delivers XML as **UTF-16 LE + BOM** (the only encoding Windows re-imports), so the tool decodes it to real text — and tells you the file must be *saved back* as UTF-16 LE, since the encoding is lost the moment an agent writes a string to disk. |
...
| `list_task_secrets` | `GET /api/tasks/:id/secrets` | **Which secrets a Cronsole-native job needs, and which it has — by name only.** Values are AES-256-GCM encrypted and **no Cronsole API returns one**, so there is nothing here that could read a value and no tool on this server that can set one. Three lists, kept apart because they mean different things: `referenced` (what the job asks for, as `${secret.NAME}` inside a url / header value / body / arg / env value), `stored` (what the task holds), and `missing` (the intersection that makes the next run **refuse to start** — `ran: false`, not a failed check). A stored secret nothing references is leftovers, not a fault. `unreadable: true` is a **third** state and never an empty list: a store exists that cannot be decrypted (`ENCRYPTION_KEY` changed), so the values are gone and must be re-entered. To fix a missing one, tell the user to set it in the app — do not ask them to paste it into the conversation. See [ADR 0003](../docs/adr/0003-per-job-secrets.md). |
...
| `untrack_task` | `POST /api/tasks/:id/untrack` | Remove a task from Cronsole **without deleting it** — the scheduled task stays on the machine and keeps running; only Cronsole's record and its Cronsole run history go, and future syncs won't re-import it. The right verb for tidying a dashboard or undoing an over-broad import. Reversible by re-importing the category. Refuses `TASKHUB_NATIVE` (exists only inside Cronsole, so nothing to keep) and a `CLAUDE_CODE` task whose routine is **declared** in the connection config (tracked *because* it's declared, so removing the row leaves the declaration and the next sync brings it back — use `disconnect_claude_routine`). A `CLAUDE_CODE` task discovered through an OAuth session and never declared works normally. Ungated. |
| `list_platforms` | `GET /api/tools/platforms` | The capability matrix: per platform, which verbs Cronsole can perform and the evidence behind each. Three states, and the middle one is the design — `verified` (has succeeded here, with the timestamp that earned it), `declared` (the route would accept it; nothing observed yet), `unsupported` (the route would refuse, because no such API exists). `unsupported` is a boundary, not a to-do. A row's **`lastFailureAt` / `lastFailureReason`** mean something narrow: Cronsole could not *perform* the verb — never that the work it performed reported bad news. A CHECK that correctly finds a missing file is the `run` verb **working**, and records a success; a failure reason describing the user's own system is a bug, not a capability limit. Each row also carries **`access`** — `controller` (Cronsole can change scheduled work there) or `observer` (it only reads). An observer is a *finished* connector: GitHub Actions and Vercel Cron are read-only on purpose, so do not plan around either gaining write verbs. **Hosted does not imply observer** — Gemini API Triggers is a `controller`, and running one there *is* the scheduled invocation, so do not carry the GitHub/Vercel assumption onto it. Call it before planning anything you are unsure is possible. |
...
| `disconnect_claude_routine` | `DELETE /api/tools/platforms/claude/routines/:id` | Forget a routine. **Not a delete** — it keeps running at claude.ai, which Cronsole has no API to stop. The stored token is discarded and cannot be recovered (claude.ai shows it once), so re-connecting means generating a new one. **Removes the tracked tasks with it** (`tasksRemoved`), which is not incidental cleanup — a Claude task is tracked *because* the routine is declared, so this is the only way to take one off the dashboard. Only reaches a **declared** routine (404s otherwise) — a task discovered through an OAuth session was never declared, so `untrack_task` is what removes it. Ungated. |
| `sync_tasks` | `POST /api/tasks/sync` | Import or refresh. Omit `categories` to REFRESH what you already track (adds nothing new); pass `categories` to IMPORT them — which also forgets prior untracks inside those categories, because naming a category is the gesture that started tracking it. A plain refresh deliberately does not, so it can never undo a deliberate removal. A result row may carry **`notes`** (what the sync *covered* — "read 9 workflows across 3 repositories, 3 scheduled"), **`warnings`** (what it could not do while still returning what it had) and **`partial`** (it saw less than the whole platform, so nothing was retired). **Read `notes` before calling `count: 0` a failure** — a source whose work is all push-triggered correctly imports nothing. |
...
| `create_gemini_trigger` | `POST /api/tasks` (`platform: GEMINI_TRIGGERS`) | Create a scheduled **prompt** for a Gemini agent. Its own tool rather than a `platform` on `create_task`, because the unit of work is a sentence and not a command — none of that tool's tokenization or no-shell guidance applies. Grants built-in tools by name and MCP servers **by preset**, so a bearer token never enters the conversation. A trigger is **immutable in place**: from here, changing a prompt means create-then-delete (a *new* task), and the tool says so — and points at the UI's **Recreate with changes**, which rebuilds the trigger while keeping the task. Returns **`promptWarnings`** — the server's preflight of the prompt (a question nobody can answer at 03:00, pasted gutter characters, mail with no recipient). Notes, not errors: the trigger is created either way, and the assistant is told to report them rather than rewrite the user's prompt. |
| `list_platform_runs` | `GET /api/tasks/:id/platform-runs` | Runs **the platform itself** recorded, including every scheduled run Cronsole never triggered. The companion to `get_task_history`, and the two stay separate on purpose: that one reads `ExecutionLog`, which holds only what Cronsole *performed*, so on a source that runs work by itself it is empty **by design**. This is where the runs actually are on Gemini, GitHub Actions, Vercel and Windows. A live read, stored nowhere, so it can fail alone. A platform publishing no history answers as a **fact, not an error** — Vercel is the permanent case. |
| `get_run_output` | `GET /api/tasks/:id/platform-runs/:runId/output` | What one run produced: its text, **the steps it took**, whatever the platform counts, and a run URL where one exists. **Read the steps, not the status** — an agent asked to email a report finishes `completed` having only written a file. One run per call (a transcript is ~90KB) and long text is truncated with the cut announced. A refusal carries its reason, and *still running* / *produced nothing* / *aged out* are three different facts. |
...
- **Read-only** (`list_*`, `get_task_history`, `list_platform_runs`, `get_run_output`, `export_task`, `convert_schedule`) — always on.
- **Creates** (`create_task`, `create_native_*`, `create_gemini_trigger`, `create_claude_routine`, `create_task_from_template`) — ungated, and reversible by deleting what they made. `create_gemini_trigger` is the one that makes an **autonomous agent**, so its description carries the prompt rules rather than assuming the caller knows them.
