...
Local clones live side by side in a maintainer's own `Tools\` folder — `CRONSOLE_REGISTRY_CLONE`
and `CRONSOLE_SITE_CLONE` name them, and an unset variable falls back to a `%TEMP%` clone:
