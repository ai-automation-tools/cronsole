...
| **`publish-frontdoor.ps1`** | Mirror the same `registry-site/` page to the public `cronsole-site` repo — Cronsole's front door at `cronsole.ai-automation-tools.dev`. One page, two hosts. Excludes `README.md` and `CNAME`, which the target repos own. |
...
| **`check-dist-fresh.mjs`** | Report whether `frontend/dist` — the bundle the reverse proxy serves — is older than the source it was built from. It is the **fourth** thing in this repo that runs stale and the only one with no keeper: a stale backend 404s, a stale agent 502s, an unbuilt `mcp-server/dist` behaves like old code, but a stale `dist` serves a complete, working, correct-looking dashboard from another day while `:7373` stays current — so the two addresses disagree and only the one nobody is looking at is wrong. **Omits itself when the proxy is down** (nothing serves `dist`, so its age is a fact about nothing) and reports `UNKNOWN` when Docker cannot be asked; neither is a pass. `--force` measures anyway. Also warns when `frontend/.env.local` sets `VITE_API_URL`, which is the one input that can still bake a wrong API origin ([#63](../docs/troubleshooting/README.md#63-the-proxied-dashboard-loads-on-the-phone-but-cannot-reach-the-backend)). Wired into `/doctor` as check 7. Cost an hour on 2026-08-23 ([#53](../docs/troubleshooting/README.md#53-the-proxied-dashboard-is-stale-while-the-dev-server-is-current)). |
