...
<p align="center">
  <em>Copy-paste prompts for driving TaskHub in natural language.</em>
</p>
...
TaskHub is built to be talked to. Whether you're asking an assistant to create a real
scheduled task, or asking one to help you work on TaskHub itself, the quality of what you get
back depends on the prompt. This library collects **ready-to-use prompts**, grouped by *how*
you're talking to TaskHub.
...
> opposite jobs: the MCP server lets an agent **use** a running TaskHub; the skill teaches an
...
```text
Using TaskHub, list my tasks whose last run failed, then show me the recent
...
```text
Using the TaskHub REST API at http://localhost:3000 with my JWT, export the
task <task-id> and save the XML, then import the template JSON in ./cleanup.json.
```

## 💡 Getting good results

- **State your timezone.** TaskHub stores every schedule as **UTC** and displays it in local
  time. When you say "7am", tell the assistant *where* — it converts for you, but only if it
  knows your zone. (See how this bit even a careful run in [task authoring](../../skills/taskhub/references/task-authoring.md#3-get-the-schedule-right).)
...
- **Destructive actions are gated.** Deleting a task over MCP only works if the operator
  enabled it (`TASKHUB_MCP_ALLOW_DESTRUCTIVE=true`); otherwise the assistant will offer to
...
| [**✍️ Task authoring & management**](../../skills/taskhub/references/task-authoring.md) | The invariants behind a good task: UTC cron, no-shell commands, folders, verification. |
| [**🧠 The `taskhub` skill**](../../skills/README.md) | What the skill knows and how it's installed. |
