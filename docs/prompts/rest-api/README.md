...
<p align="center">
  <em>Drive TaskHub directly over its REST API — including the operations MCP doesn't expose.</em>
</p>
...
TaskHub's **REST API** directly with `curl` (or any HTTP client).
...
> your user. The MCP server's `TASKHUB_TOKEN` is exactly such a token — see the
...
```text
Using curl against the TaskHub REST API at http://localhost:3000 with my bearer
token in $TASKHUB_TOKEN, GET /api/tasks and summarize my tasks by category and
status.
```

```text
Using the TaskHub REST API, fetch the run history for task <task-id>
(GET /api/tasks/:id/executions) and tell me if the last three runs succeeded.
```

```text
Using the TaskHub REST API, GET /api/tasks/folders and list only the folders I
...
```text
Using the TaskHub REST API, POST to /api/tasks to create a task that runs
...
```text
Using the TaskHub REST API, list templates (GET /api/templates), pick the
...
```text
Using the TaskHub REST API, preview how the cron "0 */4 * * *" converts before I
...
```text
Using the TaskHub REST API, export task <task-id> (GET /api/tasks/:id/export)
...
```text
Using the TaskHub REST API, export all my templates (GET /api/templates/export)
to a JSON file so I can back up my catalog.
```

```text
Using the TaskHub REST API, import the template JSON in ./my-template.json
(POST /api/templates/import) and tell me if it validated.
```

```text
Using the TaskHub REST API, turn my existing task <task-id> into a reusable
...
```text
Using the TaskHub REST API, disable task <task-id> (PATCH /api/tasks/:id/status
with { "status": "DISABLED" }) and confirm the new status.
```

```text
Using the TaskHub REST API, re-schedule task <task-id> (PATCH
/api/tasks/:id/schedule) to "0 2 * * *" — verify the trigger conversion first.
```

```text
Using the TaskHub REST API, run task <task-id> now (POST /api/tasks/:id/run) and
report the result.
```
...
| [**✍️ Task authoring & management**](../../../skills/taskhub/references/task-authoring.md) | The rules the API enforces — UTC cron, no-shell, folder refusals. |
