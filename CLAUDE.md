...
└── skills/         # 19 locally-installed skills (mirrors §6 above)
    │               #   + cronsole/ — a per-machine JUNCTION to the repo-root skills/cronsole
...
- **Restore is the one carve-out to "Cronsole creates only `\Cronsole`".** That invariant exists because folder deletion needs elevation, so a folder Cronsole creates is a door only the user can close. A restore is the user asking for *their own* tree back by name — so it may recreate folders, but only behind the explicit opt-in above, and the result **names every folder it created**. `\Cronsole` is still the only folder Cronsole ever *removes*.
