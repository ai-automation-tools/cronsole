...
│   └── src/                   #   index.ts (stdio bootstrap) · client.ts (axios) · tools.ts (23 tools;
...
Exposes **23 tools** over stdio, each a 1:1 call to a backend route — **read**: `list_tasks`, `list_templates`, `list_folders`, `get_task_history`, `export_task`, `convert_schedule`, `list_platforms` (the capability matrix — what Cronsole can do here and how it knows), `get_task_health`, `list_run_history` (cross-task), `list_claude_routines`; **create**: `create_task` (free-form command → a real task, no template), `create_native_task` (full HTTP job spec), `create_native_script_task` (run a program on the backend host), `create_task_from_template`; **act**: `run_task`; **modify**: `set_task_status`, `update_task_schedule`, `update_task_action`, `untrack_task` (drops Cronsole's row, leaves the real scheduled task running), `sync_tasks` (refresh vs import); **connect**: `connect_claude_routine` / `edit_claude_routine` (fixes a mistyped id **keeping the token** — claude.ai shows one once, so a typo must not cost a credential) / `disconnect_claude_routine` — and there is deliberately **no `create_claude_routine`**, because Anthropic exposes one routines endpoint and no create, so the tool is named for the verb it performs; **destroy**: `delete_task`. Covered by its own vitest suite (`cd mcp-server && npm test`; real tools driven through a real MCP client over an in-memory transport, stubbed HTTP client) — which by construction **cannot** prove the wrapper and the API still agree, so hand-drive a tool after changing a route it wraps.

**Destructive-op gating is tiered** (decided 2026-07-15): read-only and *reversible* verbs are ungated; only `delete_task` is, behind `CRONSOLE_MCP_ALLOW_DESTRUCTIVE=true` — and when off it is **absent from `tools/list`**, not present-and-erroring. `set_task_status` ships ungated *on purpose*: it's the honest way to park a task, and gating it would push an agent toward encoding "don't run" in the cron (troubleshooting #14) — **a gate that makes the safe path harder than the unsafe one is worse than no gate**. The gate is an **env var, not a `confirm: true` param**: a param is filled in by the model, i.e. the caller assuring itself it's sure, which is the missing deliberation rather than a substitute for it. Still REST-only: template import/export, save-as-template, agent pairing, analytics, bulk export/restore, and **favorites** (`POST` / `DELETE /api/tasks/:id/favorite`) — `compactTask` does not forward `isFavorite`, so `list_tasks` cannot read a star either. **The three bulk verbs are omitted deliberately, not pending**: friction must scale with blast radius, and the UI earns that with a typed confirmation past 25 tasks — an MCP tool has no equivalent gesture, so a bulk untrack of 254 tasks would carry less deliberation than the single-task path. Requires `cd mcp-server && npm run build`, a running backend, and `CRONSOLE_TOKEN` exported in the environment **the host was launched from** (`.mcp.json` references it as `${CRONSOLE_TOKEN}` so the committed config never holds the secret). On Windows, a newly set variable needs a **fresh terminal** — a process inherits its parent's environment, so restarting Claude Code inside an old terminal won't pick it up. If the variable is unset the host forwards the literal `${CRONSOLE_TOKEN}`, the API returns a misleading `403 Invalid or expired token`, and the server refuses to start — so the tools go *missing* rather than erroring. See [troubleshooting #8](docs/troubleshooting/README.md#8-every-mcp-tool-returns-403-invalid-or-expired-token) and the [MCP Server Guide](docs/user-guides/guides/MCP_Server_Guide.md).
...
- **"Cannot" and "has not yet" are different cells, and only the connector knows which.** `verbReachability` treats the four interface-mandated verbs (`sync`, `run`, `create`, `setStatus`) as reachable because the method must exist — true for a method that reaches a platform, false for one that is a hardcoded `{ success: false }` because no such API exists. A connector declares those in **`unsupportedVerbs`**, which outranks every other reachability rule. The distinction is the matrix's whole value: **`declared` is a promise, `unsupported` is a boundary** — reporting `create` as `declared` for Claude invited waiting for evidence that can never arrive. Only for the structurally impossible; a verb that fails because the agent is offline is still reachable, and that is what health is for. **It also picks the status code**: `verbDeclaredUnsupported` makes those refusals a `400` (the convention every optional verb already followed) instead of the `502`/`500` that means *retry, the platform is having a moment*.
- **A health check may not have a side effect, so where the only endpoint does, health comes from the run record.** Claude Code exposes one routines endpoint and it *fires the routine* — "check whether this works" and "run the user's nightly job" are the same HTTP request, so a probing `getHealth` would burn their daily run cap on every poll. `ClaudeConnector` reads the `PlatformCapability` evidence the run route already writes, which is the same doctrine `capabilitySupport` follows one layer up: **a verdict comes from what happened, never from a precondition** (troubleshooting #40, #45). It returned `HEALTHY` whenever config was non-empty before this.
- **A connection config the user composes gets its own validated route, never a generic one.** Claude is the first: Anthropic mints a bearer token *per routine* and exposes no API to list them, so the registry is typed in (`GET`/`POST`/`DELETE /api/tools/platforms/claude/routines`, `services/claudeRoutines.ts`). A generic `PUT /platforms/:platform/connection` would imply the others are configurable this way — they are not — and would accept an arbitrary blob into a field every connector trusts. **The token is write-only**: stored via `serializeConfig`, returned as `hasToken`, with no reveal endpoint (claude.ai cannot re-display it either, so a second copy would outlive its purpose). Re-adding an id **rotates** rather than 409s, because generating a token revokes its predecessor; removing the last routine **removes the connection**, so the card reads "Not connected" rather than sitting at amber *"No routines configured"*.
- **Write-only is a legitimate connector shape, and so is read-only.** Claude can `run` and cannot read; the planned GitHub Actions / Vercel Cron observers read everything and change nothing. Both earn a connector over a quick link by the same test — *does it do something a bookmark cannot?* — and both must say plainly which half they are, because the user's mental model defaults to the Windows-shaped one that does both. `syncTasks` returning user-declared config (Claude) is not a sync and is documented as not being one.
- **A connector may ship as an *observer*, and that is a finished state, not a stalled one.** Read-only (sync + health, every mutating verb absent) is legitimate and complete, because the capability matrix renders each verb as **verified / declared / unsupported** — so an observer states its limits rather than implying capabilities it lacks. This is why the strategy guardrail splits: **controllers** (anything that writes) unlock only on the reliability bar, since they can damage a user's machine; **observers** are not gated, because they cannot. The guardrail's content was never "few connectors" — it was "never imply a capability you do not have", and the matrix now enforces that structurally instead of by scarcity. *Corollary:* refusing to *show* a scheduled job because Cronsole cannot yet *control* it is the invisible-fence failure at product scale.
- **Prefer the connector that unlocks several sources over the one that unlocks one.** launchd, cron and systemd timers are all *local OS schedulers* — the same shape as Windows Task Scheduler, and one POSIX agent covers all three on the existing protocol with no new auth story. Each cloud scheduler costs its own OAuth surface, rate limit and mental model for exactly one source. That asymmetry, not popularity, sets the order in [`ROADMAP.md`](docs/ROADMAP.md) › Sources.

- **A Cronsole-native `EXEC` job inherits none of the backend's environment, and says where it runs.** Native gained a second job type on 2026-08-12 (`{executable, args[]}`, the same no-shell `StructuredAction` the Windows path uses — `services/NativeTaskExecutor.ts`). Two rules come with it, and both are the difference between a feature and a liability:
  - **The child gets an allowlist, never `process.env`.** This process holds `DATABASE_URL`, `JWT_SECRET` and the AES key that encrypts every `PlatformConnection.config`, so a scheduled script inheriting the environment could read out the key protecting every stored platform credential — defeating encryption-at-rest through the same UI that created the task. `childEnv()` passes OS essentials (PATH, SystemRoot, HOME, …) plus whatever the job spec sets *explicitly*, which is a decision someone made rather than an inheritance nobody noticed. Pinned by a test, and mutation-tested.
  - **The task states its execution host.** A native job runs wherever *the backend* runs: the user's machine on a host-run stack, and **inside the container** on the Dockerized one, against a filesystem that is not theirs — failing as "executable not found" for a file they can see in Explorer. `services/runtimeContext.ts` detects it once at boot and the New Task modal says which it is. A task that cannot say where it executes is the same class of defect as a timestamp that cannot say what it measured ([#42](docs/troubleshooting/README.md#42-the-dashboard-says-synced-7m-ago-over-a-task-list-from-yesterday)).
  - *Corollary:* "run a script" belongs on the **agent** for anything that must run as the user or survive Cronsole being down. The agent is the thing that unambiguously means "your machine"; the backend is not.
...
- **A source is finer than a platform, and the two must not be conflated.** A *platform* is what Cronsole talks to — one connector, one capability row, one `PlatformConnection`. A **source** is what the user navigates by, and they stopped matching when Cronsole-native gained a second job type: an HTTP job and a script are the same platform and genuinely different things. The key is `PLATFORM` or `PLATFORM:SUBTYPE` (`TASKHUB_NATIVE:EXEC`), derived **server-side** in `services/taskSource.ts` and sent on each task like `isSystem`, because deciding it means reading `metadata.job.jobType` and a browser-side copy of that judgement is the drift shape of #20a. Three consequences. **The Platforms tab does not split** — native HTTP and native scripts have identical capabilities because they are the same connector, so capability rows stay per-platform. **Matching is prefix-aware** (`sourceMatches`): selecting the bare `TASKHUB_NATIVE` matches both subtypes, which is what let the split ship without silently emptying every stored `defaultPlatform` and every existing link. And **the selected source stays listed even when nothing derives it**, or a pre-split link filters the list while leaving the whole bar unlit — the lit-chip problem inverted, and just as dishonest.
