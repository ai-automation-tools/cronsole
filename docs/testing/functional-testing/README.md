...
| F6.5 | **MCP tool surface** | All 6 tools (`list_tasks`, `run_task`, `list_templates`, `create_task`, `create_task_from_template`, `convert_schedule`) register, validate their inputs, filter, and render honestly — driven through a real MCP client over an in-memory transport | ✅ `mcp-server/src/__tests__/tools.test.ts` |
