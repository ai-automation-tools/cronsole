...
*Last updated: 2026-07-13 (P1 complete; P2 rolling. Shipped since: the Windows management pass (enable/disable → schedule edit → action/settings edit), Templates-tab work (view selector, Resources menu, honesty pass, favorites), and the **template registry** — the catalog is now a decoupled, versioned, hosted registry (`backend/src/catalog/` + `registry/` → the public `taskhub-registry` repo) synced into the DB on boot + interval. See `docs/ROADMAP.md` and `docs/reports/templates/Registry_Schema_v1.md`.)*
