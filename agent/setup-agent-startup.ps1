# Cronsole Agent Startup Registration Script
...
$Description = "Cronsole Local Control Plane Agent - Handles WebSocket communication with Cronsole backend."
...
# apply to time-limit kills), leaving Cronsole offline until the next logon.
...
foreach ($Task in $ExistingTasks) {
    Write-Host "Removing existing CronsoleAgent task from path $($Task.TaskPath)..." -ForegroundColor Yellow
...
Write-Host "Starting Cronsole Agent task in the background..." -ForegroundColor Cyan
Start-ScheduledTask -TaskName $TaskName -TaskPath $TaskPath

Write-Host "The Cronsole agent will now launch automatically in the background whenever you log into Windows." -ForegroundColor Green
