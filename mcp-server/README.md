...
- **Read/preview vs. side-effecting.** `list_*` and `convert_schedule` are safe to call
  freely; `run_task`, `create_task`, and `create_task_from_template` cause real effects on the
  user's machine (running / registering Windows tasks) — same guardrails as the dashboard.
