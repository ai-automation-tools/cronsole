 ...
 * overwrite works: \Work\Backup and \Cronsole\Backup are different tasks, while
 * two \Work\Backup are the same one. So the guard takes the target folder — it
 * used to assume \Cronsole\, and once folders became selectable that assumption
 ...
 *
 * `folder` defaults to \Cronsole for callers that haven't opted into choosing
...
export async function assertWindowsTaskNameAvailable(
  ...
  // character — a path like \Cronsole\x silently never matches. So match
