...
- **Friction has to scale with the blast radius, and a scope is what makes that possible.** A confirmation that is the same dialog at 3 tasks and at 254 does not get harder as the operation gets bigger, so habit built on small batches carries straight through to a machine-wide one — which is the actual gap, not a missing dialog (bulk status and untrack always confirmed). Two rules follow. **Past `TYPE_TO_CONFIRM_THRESHOLD` (25) the confirmation costs a deliberate act** — typing the count — because that is the smallest gesture muscle memory cannot produce; and **the dialog states the scope in words**, since `254 selected` is not something a reader can check while *"every tracked task of yours"* is. That is why the Mass Actions console (`components/tools/MassActionsTool.tsx`, `utils/massActions.ts`) is scope-first rather than a bigger checkbox. Two corollaries that are easy to get backwards: **relocating a control is not a safety measure** — friction by obscurity wears off, so the dashboard keeps its selection for small batches and Untrack stays beside Delete as the safe neighbour it must not be harder to reach than; and **a UI must not offer a selection its routes would refuse** — `Select all 269` built a batch every bulk button then 400'd on, so select-all is capped at `MAX_TASKS_PER_BULK` and hands off to the console, which chunks. Chunking honours that ceiling rather than arguing with it: the bound is correctness (each Windows task can cost its full ~15s agent timeout), and batching also buys progress and a stopping point one long request could never have.
