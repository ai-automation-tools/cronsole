...
<p align="center">
  <em>Work on TaskHub's codebase with the <code>taskhub</code> Agent Skill loaded.</em>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/surface-taskhub_skill-8B5CF6?style=for-the-badge" alt="taskhub skill">
  <img src="https://img.shields.io/badge/audience-builders-F97316?style=for-the-badge" alt="Builders">
</p>

---

These prompts are for an assistant working **inside the TaskHub repo** with the `taskhub`
...
> explicitly with `/taskhub`. It lives canonically at
> [`skills/taskhub/`](../../../skills/README.md) and is teaching material, **not** a way to
...
```text
Using the taskhub skill, give me the architecture in a paragraph: the four
long-running processes, who dials out to whom, and why the agent can't be
containerized.
```

```text
Using the taskhub skill, explain how a created Windows task's cron schedule
becomes a real Task Scheduler trigger, and where that conversion can go lossy.
```

```text
Using the taskhub skill, what are the non-negotiable invariants I have to
respect if I touch the agent command-signing path?
```

```text
Using the taskhub skill, walk me through the "two AI surfaces" — the MCP server
...
```text
Using the taskhub skill, add a new template that runs a weekly `npm audit` and
...
```text
Using the taskhub skill, explain the core vs. extended tiers and what I flip to
promote a template into the built-in set. What must NOT reach the database?
```

```text
Using the taskhub skill, one of my templates fails to resolve through the Apply
...
```text
Using the taskhub skill, I want to add an MCP tool that lists a task's next few
...
```text
Using the taskhub skill, I changed the response shape of GET /api/tasks. What in
...
```text
Using the taskhub skill, my agent shows OFFLINE in the dashboard even though
it's clearly running. What's the likely cause and the fix?
```

```text
Using the taskhub skill, I added a new backend route but a live request 404s
while the tests pass. What's going on?
```

```text
Using the taskhub skill, a new agent command 502s with "Agent timeout" after
~15 seconds, but DB-only routes work fine. Diagnose it.
```

```text
Using the taskhub skill, my .ps1 parses fine in pwsh 7 but throws "Unexpected
...
```text
Using the taskhub skill, I'm about to commit a change that adds a backend route
and an MCP tool. Which records and mirror surfaces must I update in the SAME
change, per the repo's rules?
```

```text
Using the taskhub skill, I just solved a confusing setup failure that cost me an
...
| Resource | Why |
|:---|:---|
| [**🧠 The `taskhub` skill**](../../../skills/README.md) | What the skill knows, how it's installed (the per-machine junction). |
