...
| `run_task` | `POST /api/tasks/:id/run` | Trigger a task now by its Cronsole id (Windows → signed agent run; native → backend runs it). |
...
| `create_task_from_template` | `POST /api/templates/:id/apply` | Create a real task from a template — server fills placeholders from `parameters`, converts the cron, and registers it. Optional `folder` picks the Windows Task Scheduler folder (default `\Cronsole`; `\Microsoft\` refused). Only Windows + Cronsole-native are creatable today. |
...
| `list_claude_routines` | `GET /api/tools/platforms/claude/routines` | The Claude routines connected to Cronsole, **without tokens** (`hasToken` only). Not a read of claude.ai — Claude Code has no list API, so this is what has been declared here and nothing else. |
| `connect_claude_routine` | `POST /api/tools/platforms/claude/routines` (+ `POST /api/tasks/sync`) | Store a routine's id and token so Cronsole can fire it, then import it. **Connect, not create** — Anthropic exposes no create endpoint. Re-connecting an id **replaces** the stored token, which is the rotation path (generating at claude.ai revokes the previous one). Takes a live credential as a parameter, so it lands in the host's transcript — prefer the UI when a human is present. Ungated. |
