...
| F1.8f | **Health score is evidenced** | Every signal names its source; **no evidence scores `unknown`, never `ok`**; disabled is informational and weighs 0; Task Scheduler status codes are not read as exit codes; duration drift never raised for a fire-and-forget Windows trigger; system tasks flagged so the personal lens applies | ✅ `taskHealth.test.ts`, `run-history.integration.test.ts`, `NeedsAttentionPanel.test.tsx` |
