...
- **Plan management:** roadmap-driven (phases complete & archived). Open work is tracked in [`docs/ROADMAP.md`](docs/ROADMAP.md). **P0 security hardening is complete** (2026-07-09: agent WebSocket auth, config encryption at rest, multi-tenancy route scoping + JWT rotation, structured command handling — no more `cmd.exe /c`); the near-term focus is P1 correctness. 2026-07-10 shipped three more P1 items: **server-side per-token template substitution** (Apply sends raw parameter values; the backend owns `{{placeholder}}` substitution — a parameter is always exactly one argument), the **backend hygiene sweep** (shared Prisma singleton in `backend/src/db.ts`, one error middleware + Zod validation on every mutating route body, batched sync upserts), and the **agent-auth hardening follow-ups** (the `task:create` trigger is now signed into the HMAC, per-command replay guard, and a session-key reconnect overlap on the agent). **The QA suite is the last P1 item and is underway**: the **backend integration suite against real Postgres** shipped (21 tests — the P0 IDOR/multi-tenancy regressions, auth flow, config encryption-at-rest, batched sync upserts — via an extracted `createApp()` + supertest on a dedicated `taskhub_test` DB, wired into CI; run with `npm run test:integration`), and **Playwright E2E is scaffolded** (`frontend` `@playwright/test` + a green dashboard smoke test; run with `npm run test:e2e` against a live stack). Remaining QA: the full E2E scenario set (needs a mock agent for the Run Now / apply / offline flows), resilience/soak tests, and the security audit + performance report. Known gap flagged during the integration work: the archived Test_Plan's `429` login rate-limit isn't implemented (tracked in the Go-public checklist).
...
*Last updated: 2026-07-10 (P1: server-side template substitution + backend hygiene sweep + agent-auth hardening follow-ups shipped; QA suite underway — backend integration suite (real Postgres) + Playwright E2E scaffold done, full E2E/soak/security-audit remaining; see `docs/ROADMAP.md`).*
