...
- **Plan management:** roadmap-driven (phases complete & archived). Open work is tracked in [`docs/ROADMAP.md`](docs/ROADMAP.md). **P0 security hardening is complete** (2026-07-09: agent WebSocket auth, config encryption at rest, multi-tenancy route scoping + JWT rotation, structured command handling — no more `cmd.exe /c`); the near-term focus is P1 correctness. 2026-07-10 shipped two more P1 items: **server-side per-token template substitution** (Apply sends raw parameter values; the backend owns `{{placeholder}}` substitution — a parameter is always exactly one argument) and the **backend hygiene sweep** (shared Prisma singleton in `backend/src/db.ts`, one error middleware + Zod validation on every mutating route body, batched sync upserts). Remaining P1: agent-auth hardening follow-ups + the QA suite (integration/E2E).
...
*Last updated: 2026-07-10 (P1: server-side template substitution + backend hygiene sweep shipped; see `docs/ROADMAP.md`).*
