...
| **The Tools tab has never been driven in a browser** | Bulk export and restore are covered by unit tests, integration tests, and (for restore) a live API pass — but nobody has clicked through the actual cards. The file pickers in particular cannot be automated: `showDirectoryPicker` is a native dialog ([#24](../troubleshooting/README.md#24-showdirectorypicker-throws-must-be-handling-a-user-gesture-after-an-await)), and so is the restore file/folder input. **Execution analytics** (2026-07-31) joins the same gap, and it is the card most exposed to it: this feature family has twice been fixed by things only a **real 354-task population** revealed — system tasks dominating a "needs attention" list, and a Windows result code read as an exit code — neither of which any fixture contained | This doc + [UAT U5.4a](uat/README.md), [U5.4b](uat/README.md) |
