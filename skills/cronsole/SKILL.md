...
| **A rename pass disables its own back-compat shim** | A rename lands, every suite is green, and a working setup stops working — MCP tools vanish, webhooks stop firing. | The shim held the old name as a **literal**, so the pass rewrote it into `CRONSOLE_x ?? CRONSOLE_x` — a tautology that compiles and reads right — **and rewrote the guarding tests to match**, so they still passed. Assemble the legacy name from parts (`['TASK','HUB'].join('')`) so a text pass can't match it, and mutation-check the fallback. Same family: **a PowerShell script ending in `Stop-Process -Id $PID` kills its caller** — invoke it as a child process and verify its effect by querying the system, not by an exit code. ([#32](../../docs/troubleshooting/README.md#32-a-rename-pass-silently-disables-the-back-compat-it-just-added--and-rewrites-the-tests-too)) |
