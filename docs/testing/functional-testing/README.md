...
| F6.5 | **MCP tool surface** | Every registered tool registers, validates its inputs, filters, and renders honestly — driven through a real MCP client over an in-memory transport. **Count and names are deliberately NOT listed here.** They were, as "a deliberate tripwire" naming 14 tools — and the surface reached **29** without this row changing, so the tripwire recorded a number nobody re-read rather than obligating anything. The real tripwire is mechanical: `grep -c 'server.registerTool' mcp-server/src/tools.ts` against the two tool tables (`mcp-server/README.md`, `MCP_Server_Guide.md`), which `/sync-surfaces` diffs by name. `delete_task` is the only gated verb (`CRONSOLE_MCP_ALLOW_DESTRUCTIVE`) and is **absent** from `tools/list` when off | ✅ `mcp-server/src/__tests__/tools.test.ts` |
