...
| **TaskHub creates exactly one folder unasked: its own `\TaskHub`** | The only one it also *prunes*. **Never create what you cannot remove** — deleting a Task Scheduler folder needs elevation, so any other folder TaskHub created would be a one-way door only the user could close by hand. Every other folder must already exist; a create into a missing one is refused honestly. A user's folder persisting is *correct* — it's theirs. **One carve-out, added 2026-07-28: restore** (`task:import`) may recreate a missing folder, because there the user is asking for their own tree back by name — but only behind an explicit opt-in that is **inside the signature**, and the result must name every folder it created. `\TaskHub` remains the only folder TaskHub ever *removes*. |
...
| **`ExecutionLog` records runs TaskHub *performed*, not runs that *happened*** | Rows come from exactly two places: `POST /api/tasks/:id/run` and `NativeScheduler`. **A Windows task firing on its own schedule writes nothing**, and a manual Windows run's `SUCCESS` means *"the agent started it"* — fire-and-forget, with `durationMs` timing the trigger, not the work. So a Windows task's real health lives in **Windows** (`lastTaskResult`, `lastRunTime`, `numberOfMissedRuns`, carried on the sync snapshot); only TaskHub-native tasks can be judged from `ExecutionLog`. Anything reading it as a complete run history is wrong at dashboard scale. |
