...
/**
 * Pick folders on the machine and start tracking what is in them.
 *
 * This is the half of the old Import modal that reads a **source**, and it lives
 * under **Sync** now: Import takes a file, Sync reads a machine. The chooser
 * that used to ask which of the two you meant is gone, because the two are
 * different controls rather than two answers to one button.
 *
 * **Nothing here creates a task.** These tasks exist and run whether or not
 * Cronsole knows about them; adopting one is a row in a database. That is what
 * makes selective import safe to offer at all — and why it is *selective*: on a
 * real machine 257 of 352 tasks are Windows' own, so "everything" is the wrong
 * default and always was.
 *
 * The one thing this shares with a plain refresh is the route. `POST
 * /tasks/sync` with explicit `categories` **forgets the untracks inside them**,
 * because naming a folder is the same gesture that started tracking it; the
 * refresh (`scope: 'tracked'`) deliberately does not, so a routine sync can
 * never undo a deliberate removal. Two gestures, two request shapes, on purpose.
 */
...
interface DiscoveredCategory {
  ...
  count: number;
  /**
   * Tasks in this category the user removed from Cronsole, which adding it
...
interface SyncSourcesModalProps {
  onClose: () => void;
  /** Commit: the categories to adopt, sent as `POST /tasks/sync { categories }`. */
  onAdd: (categories: string[]) => void;
}

export const SyncSourcesModal = ({ onClose, onAdd }: SyncSourcesModalProps) => {
  const { data: discovery, isLoading, error } = useQuery<DiscoveredPlatform[]>({
    ...
    // Fetched on mount, and only because this modal is now opened *by asking for
    // it*. Discovery is an agent round trip that can take its full timeout and
    // fails outright when the agent is offline, so it must never be spent on
    // someone who came here to do something else.
  });
  ...
  useEffect(() => {
    ...
    // seeds the initial selection, and re-running it when the commit writes the
    ...
    >
        <header className="p-6 border-b border-border flex justify-between items-center bg-surface/50">
          <div className="min-w-0">
            <h2 id="sync-sources-title" className="text-xl font-bold">Add tasks from this machine</h2>
            {/* States the consequence, because it is the thing that separates
                this from the other file-shaped gesture in the app: adopting a
                task changes nothing about the machine. */}
            <p className="text-xs text-subtle-foreground mt-1 leading-relaxed">
              Pick folders to start tracking. Nothing is created — these tasks already run.
            </p>
          </div>
          <div className="flex items-center gap-1 shrink-0">
            <HelpButton topic="import" size="md" />
            <button onClick={onClose} aria-label="Close" title="Close" className="p-2 hover:bg-muted rounded-full text-subtle-foreground transition-colors"><XCircle size={20} /></button>
          </div>
        </header>
            ...
            <div className="flex items-center gap-2">
              ...
              {settings.lastImportCategories !== null && (
                <span
                  ...
                  title="Your last selection, not everything on the machine."
                >
                  from last time
                </span>
              )}
                  ...
                  <div className="grid gap-2">
                    {platform.categories.map((cat) => (
                      <label key={cat.name} className={`flex items-center justify-between p-3.5 rounded-2xl border transition-all cursor-pointer group ${selected.includes(cat.name) ? 'bg-primary/5 border-primary/30' : 'bg-background/50 border-border hover:border-foreground/20'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          {/*
                            Adding a category forgets the untracks inside it, so
                            tasks the user removed on purpose come back. That is
                            correct — asking for the folder IS asking for what is
                            in it — but it has to be said before the click, not
                            discovered after it.
                          */}
                          {!!cat.excludedCount && (
                            <span
                              className="text-[10px] font-bold px-2 py-0.5 rounded-lg border bg-warning/10 border-warning/30 text-warning-text"
                              title={`${cat.excludedCount} task${cat.excludedCount === 1 ? '' : 's'} you removed from Cronsole will be tracked again if you add this category.`}
                            >
        ...
        <footer className="p-6 bg-background border-t border-border space-y-3">
          ...
          <p className="text-xs text-center text-muted-foreground" data-testid="import-preview">
            {preview.folders === 0 ? (
              'Nothing selected — pick at least one category to add.'
            ) : (
              <>
                This will track <span className="font-bold text-foreground">{preview.tasks}</span>{' '}
          ...
          <div className="flex gap-4">
            ...
            >
              Track {preview.tasks} task{preview.tasks === 1 ? '' : 's'}
            </button>
          </div>
        </footer>
    </Modal>
