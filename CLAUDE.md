...
**Local path:** `D:\AI_Agents\Projects\Mikes_AI_Lab\Repos\Live_Apps\taskhub` — **still `taskhub`, permanently.** Every scheduled task on the machine embeds this as an absolute path, and the directory name is not user-visible branding. Stage 3 renamed the *repos* and left this alone on purpose; there is no later stage that moves it. *(Promoted from `Other-Repos/` to a top-level workspace repo on 2026-06-01.)*
