...
| F1.8 | **Export a task — native** | Windows → native Task Scheduler XML (**UTF-16 LE + BOM**); Cronsole-native → JSON. The default, and the only one that restores *this* task onto *this* platform | ✅ `task-export.integration.test.ts` |
| F1.8g | **Export a task — portable** | `?format=template` returns a Registry v1 template built by the **same `buildTemplateFromTask`** save-as-template uses, and **writes nothing** (the count is asserted). The cases that earn the second format are where it beats the first: it works with the **agent offline** (where native 502s) and for **`CLAUDE_CODE`** (where native 400s). Refusals name the fact that stopped them — no cron-expressible schedule, multiple actions — and an unknown `format` names the two that exist. **Records no capability**: nothing was asked of the platform, so marking `export` verified would be a verdict without evidence | ✅ `task-export.integration.test.ts` · `TaskExportMenu.test.tsx` |
| F1.8h | **The portable file is actually importable** | The export is fed straight to `POST /api/templates/import` and accepted, landing as `managed: false`. **Asserting the shape alone would prove nothing** — the importer is the only judge of valid Registry v1, and without this, export and import could drift into two dialects with every suite green | ✅ `task-export.integration.test.ts` |
...
| F5.13 | **Export names each format by what it is for** | The task modal's Export is a menu labelled *"restores onto any Windows PC"* vs *"recreate this task on any install"*, not *"XML"* vs *"JSON"* — both are files and the extension is the half a reader cannot act on. It states the portable format's **loss before the click**, describes "native" per platform (they are not one thing), and on a platform with no fetchable definition it **says so and still offers portable** rather than hiding the button. Portalled, because the modal panel is `overflow-hidden` | ✅ `TaskExportMenu.test.tsx` (7) |
