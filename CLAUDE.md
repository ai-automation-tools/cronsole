...
- **The public registry repo is cloned locally** at `D:\AI_Agents\Projects\Mikes_AI_Lab\Repos\Tools\taskhub-registry` (remote: [`michaelschecht/taskhub-registry`](https://github.com/michaelschecht/taskhub-registry), served at `https://mikesailab.com/taskhub-registry`). It is a **separate, independent git repo** — NOT a submodule of this one (`registry/` here is a normal tracked folder). The source of truth is `taskhub/registry/` in this repo; `publish-registry.ps1` copies `index.json` + `templates/` into that clone and pushes. The public repo **owns its own `README.md`** (a house-style front page) — publish no longer overwrites it, so the two READMEs (`registry/README.md` here = dev folder note; the clone's = public front page) diverge on purpose.
