  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pt-4 border-b border-slate-800 flex gap-1">
          <button
            onClick={() => setActiveTab('overview')}
            className={`px-4 py-2 rounded-t-lg text-xs font-bold transition-all border-b-2 ${activeTab === 'overview' ? 'text-blue-400 border-blue-500' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
          >
            Overview
          </button>
          <button
            onClick={() => setActiveTab('runs')}
            className={`px-4 py-2 rounded-t-lg text-xs font-bold transition-all border-b-2 flex items-center gap-1.5 ${activeTab === 'runs' ? 'text-blue-400 border-blue-500' : 'text-slate-500 border-transparent hover:text-slate-300'}`}
          >
        ...
        {activeTab === 'runs' && (
          <div className="p-6 overflow-y-auto flex-1 text-slate-300 space-y-3">
            {DEMO_MODE ? (
              <div className="text-xs text-slate-500 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 flex items-center gap-2">
                <Info size={14} className="text-blue-500 shrink-0" /> Run history isn't available in the demo.
              </div>
            ) : executionsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-slate-500 gap-2">
                ...
                <p className="text-xs text-slate-600">Manual runs and TaskHub-scheduled fires will appear here.</p>
              </div>
            ) : (
              executions.map(run => (
                <div key={run.id} className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs text-slate-400 font-mono">
                      ...
                      {run.durationMs != null && (
                        <span className="text-slate-600"> · {run.durationMs >= 1000 ? `${(run.durationMs / 1000).toFixed(1)}s` : `${run.durationMs}ms`}</span>
                      )}
                    </span>
                  </div>
                  {run.log && (
                    <pre className="text-[10px] text-slate-400 font-mono whitespace-pre-wrap break-all bg-slate-900/60 rounded-lg p-2.5 border border-slate-800/60 max-h-28 overflow-y-auto">
                      {run.log}
                    </pre>
                  )}
                </div>
